...
def _MakeSavedModel(export_dir, checkpoint_id):
  ...
  tf.io.gfile.mkdir(os.path.join(checkpoint_dir, 'variables'))
...
class CheckpointExportListener(tf.test.TestCase):
  ...
  def tearDown(self):
    tf.io.gfile.rmtree(self._test_dir)
  ...
  def testCheckpointExportListener(self):
    ...
    self.assertTrue(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 1)))
  ...
  def testCheckpointExportListenerGC(self):
    ...
    self.assertTrue(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 5)))
    self.assertFalse(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 2)))
...
class LaggedCheckpointListenerTest(tf.test.TestCase):
  ...
  def testEmptyDir(self):
    ...
    listener.after_save(None, 10)
    self.assertTrue(
        tf.io.gfile.exists(_CheckpointDir(self._lagged_export_dir, 1)))
    self.assertTrue(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 1)))

    listener.after_save(None, 11)
    self.assertTrue(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 2)))
    # Lagged policy has not updated.
    self.assertFalse(
        tf.io.gfile.exists(_CheckpointDir(self._lagged_export_dir, 2)))

    listener.after_save(None, 12)
    # Lagged policy updates the lagged_dir.
    self.assertTrue(tf.io.gfile.exists(_CheckpointDir(self._export_dir, 3)))
    self.assertTrue(
        tf.io.gfile.exists(_CheckpointDir(self._lagged_export_dir, 2)))
